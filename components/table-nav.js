class TableNav extends PageNav {
  ...
  static async onLoad() {
    ...
    dbInfo.tables.forEach(
      (tableName) => (PageNav.navs = this.create(tableName)),
    );
  }
  ...
  static onUpload(event) {
    const { name: tableName } = event.detail;
  ...
  #removeDBTable = () =>
    new Promise((res, rej) => {
      ...
      dbRequest.onupgradeneeded = async () => {
        ...
        const nav = Object.values(PageNav.navs).find(
          (n) => n.dataset.name === tableName,
        );
        delete PageNav.navs[nav.slugify()];
        ...
        const slug = PageNav.default.slugify();
